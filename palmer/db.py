...
if _DATABASE_URL:
    ...
    import sqlite3
    _DB_PATH = Path(__file__).resolve().parents[1] / "palmer.db"  # repo root, not inside the package
    PH = "?"
...
def save_message(phone: str, role: str, content: str, kind: str | None = None):
    ...
    "watch", "price", "flight", "reminder", "reply", "city_ask"). Optional so every
...
def get_all_profiles() -> list[tuple[str, dict]]:
    ...
    user — one connection each, N+1 per tick across the morning, followup and
    watch jobs."""
    conn = _conn()
    cur = conn.cursor()
