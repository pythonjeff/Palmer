...
from palmer.db import get_profile, upsert_profile, save_artifact, get_artifact
from palmer.artifacts import new_token
...
def _fetch_weather(profile: dict) -> dict | None:
    from palmer.weather import weather_snapshot
...
def _commute_depart_at(commute: dict, tz_name: str | None, now=None):
    ...
    from palmer.smstext import _normalize_hhmm
    from palmer.timeutil import local_now, valid_zone
...
def _fetch_traffic(profile: dict) -> dict | None:
    from palmer.traffic import traffic_snapshot
...
def _fetch_prices(profile: dict, previous: list[dict] | None = None) -> list[dict]:
    ...
    from palmer.datafeeds import price_snapshot
...
def _fetch_scores(profile: dict) -> list[dict]:
    ...
    This is the same read the morning update and the check-in make
    ...
    if not teams:
        return []
    from palmer.sports import team_day
...
def _fetch_headlines(profile: dict) -> list[dict]:
    ...
    """
    from palmer.datafeeds import _search_raw
    from palmer.morning import _is_directive, _rotated_topics, _WEATHER_KEYWORDS, _TRAFFIC_KEYWORDS
...
def _fetch_opening(profile: dict) -> list[dict]:
    ...
    if (profile.get("morning_prefs") or {}).get("opening") is False:
        return []
    from palmer.opening import opening_snapshot
    try:
...
def _tracking(phone: str, profile: dict | None = None) -> dict:
    ...
    from palmer.db import get_user_watches, get_user_price_watches
    try:
