...
def start(phone: str) -> str | None:
    ...
    if not app_url:
        return None
    try:
        from palmer.home import home_token, load, save
...
def apply(token: str, payload: dict, form: dict) -> bool:
    ...
    is the one build."""
    from palmer.home import save
    from palmer.userprofile import _apply_profile_updates
    from palmer.db import get_profile
    ...
    from palmer.db import upsert_profile
...
def split_follows(text: str) -> list[str]:
    ...
    for raw in re.split(r"[,\n;]+", text or ""):
        ...
        if not item:
            continue
        try:
            from palmer.agent import _normalize_price_topic
...
def _build_async(phone: str) -> None:
    ...
    def _run():
        try:
            from palmer.home import rebuild
