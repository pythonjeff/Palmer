...
from palmer.llm import client, SONNET_MODEL
from palmer.prompts import SYSTEM_PROMPT
from palmer.tools_def import TOOLS
from palmer.smstext import _sms_clean, _normalize_hhmm
from palmer.userprofile import topic_already_covered
from palmer.weather import _get_weather
from palmer.datafeeds import _search, _get_price, _get_gif, _fetch_media
from palmer.userprofile import _update_profile, _consolidate_history
...
def _prompt_safe_profile(profile: dict) -> dict:
    ...
    from palmer.timeutil import local_today
    from palmer.userprofile import fresh_profile_for_prompt
    safe = fresh_profile_for_prompt(profile, local_today((profile or {}).get("timezone")))
    topics = safe.get("morning_topics")
    if topics:
        from palmer.morning import _is_directive
...
def base_system() -> str:
    ...
    constant itself."""
    from palmer.timeutil import clock_block
...
def _build_system(phone: str, include_recent: bool = False, is_new_user: bool = False) -> str:
    ...
    from palmer.tapback import reaction_block
    ...
    try:
        from palmer.db import get_pending_reminders
        from palmer.timeutil import valid_zone, _zone
...
def _resolve_asset(asset: str) -> str:
    ...
    from palmer.tickers import resolve_asset_name, resolve_company_ticker
    if not asset:
        return asset
...
def _normalize_price_topic(topic: str) -> str:
    ...
    from palmer.tickers import resolve_topic_asset, resolve_company_ticker, looks_like_price_topic
...
def _city_from_weather_topic(topic: str) -> str | None:
    ...
    from palmer.morning import _infer_city_from_topics
...
def _apply_opening_kinds(profile: dict, tool_input: dict, updates: dict) -> str:
    ...
    from palmer.opening import ALL_KINDS, KIND_WORDS
...
def _normalize_due_at(phone: str, raw: str) -> tuple[str | None, str, str | None]:
    ...
    from palmer.db import _parse_due
    from palmer.timeutil import valid_zone, _zone
...
def _finalize(text: str, system: str, messages: list, gif_url):
    ...
    from palmer.guards import (redirects_elsewhere, leaks_deliberation, denies_capability,
    ...
    for iteration in range(TOOL_ITERATION_CAP):
        ...
        def _dispatch(b):
            ...
            if b.name == "web_search":
                ...
                from palmer.weather import (resolve_weather_location, ambiguous_location,
                ...
                if not resolved:
                    ...
                    try:
                        from palmer.home import invalidate
                ...
                from palmer import traffic as _traffic
                ...
                if b.input.get("enabled") is True and not topics:
                    from palmer.morning import default_topics
                ...
                from palmer.page import DEFAULT_SECTION_ORDER, SECTION_WORDS
                profile = get_profile(phone_number)
                prefs = dict(profile.get("morning_prefs") or {})
                unknown: list[str] = []
                ...
                from palmer.db import cancel_reminders_named
                ...
                from palmer.shopping import search_shopping
                ...
                from palmer.shopping import browse_shop
                result = browse_shop(b.input["query"])
            elif b.name == "search_flights":
                from palmer.flights import search_flights
                ...
                from palmer.sports import find_teams, live_mode, LIVE_MODES, FOLLOW_MAX as TEAM_MAX
                profile = get_profile(phone_number)
                ...
                from palmer.sports import LIVE_MODES
                profile = get_profile(phone_number)
                current = [dict(t) for t in (profile.get("followed_teams") or [])]
                ...
                from palmer.sports import find_teams, team_game, describe
                ...
                from palmer.shows import find_shows, ambiguous_shows, describe_show, FOLLOW_MAX
                profile = get_profile(phone_number)
                ...
                from palmer.db import save_flight_watch, FLIGHT_WATCH_MAX
                ...
                from palmer.db import cancel_flight_watches
                ...
                from palmer.hotels import search_hotels
                ...
                from palmer.home import ensure_fresh
                profile = get_profile(phone_number)
                ...
                if not profile.get("city"):
                    from palmer.onboard import start as _setup_start
