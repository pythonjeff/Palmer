...
from palmer.netutil import _http_get_json_retry
...
def _grid_max_today(series: dict, tz: str | None) -> float | None:
    ...
    from palmer.timeutil import local_today as _lt
    target = _lt(tz)
    vals = []
    for v in (series or {}).get("values") or []:
        try:
...
def _nws_report(lat: float, lon: float, resolved: str, when: str, when_lower: str,
    ...
    from palmer.timeutil import valid_zone, local_today as _lt
...
def _proven_source(city: str) -> str | None:
    ...
    try:
        from palmer.wxaudit import best_source
