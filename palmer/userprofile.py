...
from palmer.llm import client, HAIKU_MODEL, _parse_json
from palmer.prompts import EXTRACT_PROMPT, CONSOLIDATE_PROMPT
...
def _eager_build_home(phone: str) -> None:
    ...
    try:
        from palmer.home import rebuild, load, home_token
...
def _apply_profile_updates(phone: str, profile: dict, updates: dict) -> dict:
    ...
    from palmer.timeutil import valid_zone
...
def _user_already_covered(phone: str, candidate: str, window_hours: float = 12) -> bool:
    ...
    from palmer.db import get_recent_user_messages
...
def _is_duplicate_subject(phone: str, new_text: str, window_hours: float = 6) -> bool:
    ...
    from datetime import datetime, timezone, timedelta
    from palmer.db import get_recent_assistant_messages
    ...
    from palmer.guards import near_duplicate
