...
from palmer.agent import _build_system
from palmer.llm import client, HAIKU_MODEL, SONNET_MODEL
from palmer.smstext import _sms_clean
from palmer.weather import _weather_report
from palmer.datafeeds import _search_raw, _get_price
from palmer.userprofile import _derive_timezone
from palmer.db import get_profile, upsert_profile, get_all_profiles, save_message, get_history, claim_daily_guard
from palmer.traffic import get_city_traffic, get_travel_time
...
def default_topics(city: str | None) -> list[str]:
    ...
    place = city
    try:
        from palmer.opening import _metro
...
def _price_asset_for_topic(topic: str) -> str | None:
    ...
    from palmer.tickers import resolve_topic_asset
...
def _topic_digest(topic: str) -> str | None:
    ...
    silently skip."""
    from palmer.sources import canonical_domain
...
def _gather_morning_data(profile: dict) -> list[str]:
    ...
    try:
        from palmer.trends import adjacent_story
...
def _recent_assistant_texts(phone: str, n: int = 4) -> list[str]:
    ...
    from palmer.db import get_recent_messages_of_kind
...
def _local_now(tz_name: str) -> datetime:
    from palmer.timeutil import local_now
...
def score_lines(payload: dict) -> list[str]:
    ...
    from palmer.sports import result_line
    out = []
...
- Palmer's voice. Plain ASCII, no emoji, no markdown, no bullets, no sign-off."""
    ...
    from palmer.guards import repeats_opening
...
def _compose_morning(phone: str) -> tuple[str, bool]:
    ...
    from palmer.home import ensure_fresh, load, home_token
...
def send_morning_messages():
    from palmer.sms_util import send_sms
