...
from palmer.agent import get_reply, save_assistant_turn
from palmer.smstext import shorten_message
from palmer.morning import generate_morning, send_morning_messages, send_missing_data_asks
from palmer.followup import run_followups
from palmer.db import get_profile, upsert_profile, save_message, get_history, HISTORY_LIMIT
...
from palmer.sms_util import ensure_sms, send_sms, FALLBACK_SMS
from palmer.tapback import parse_reaction, record_reaction, interpret_reaction, learn_from_reactions
...
from palmer.scorewatch import run_score_alerts
...
async def artifact_png(token: str):
    ...
    from palmer.artifacts import load, render_png
    payload = load(token)
    if payload is None:
...
async def artifact_page(token: str):
    ...
    from palmer.artifacts import load, image_url, page_url
    from palmer.page import render
    payload = load(token)
    if payload is None:
        raise HTTPException(status_code=404)
    return FileResponse(
...
async def home_png(token: str):
    ...
    preview outright."""
    from palmer.home import load, refresh_stale
    from palmer.artifacts import render_png
    payload = load(token)
    if payload is None:
        raise HTTPException(status_code=404)
    ...
    from palmer.artifacts import _card_fingerprint
...
async def home_setup_submit(token: str, request: Request):
    ...
    from palmer.home import load
    from palmer.onboard import apply, needs_setup
    payload = load(token)
    if payload is None:
        raise HTTPException(status_code=404)
...
async def home_page(token: str):
    ...
    from palmer.onboard import needs_setup, render_setup, render_building
    if needs_setup(payload):
        return FileResponse(
...
async def preview_morning(phone: str, full: bool = False):
    ...
    Defaults to the real thing — weather, commute, and 1-2 opening highlights
    plus their page link. Pass full=1 for the long-form text briefing, which
    is now only the fallback for when the page can't be built."""
    if full:
        return PlainTextResponse(generate_morning(phone))
    from palmer.morning import _compose_morning
