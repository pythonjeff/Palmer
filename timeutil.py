...
("today's weather", "did we already text them today", "is it time for their
evening update") has to be evaluated in the user's own timezone.

Modules that already had ad-hoc UTC calls (agent.py's weather day resolver
among them) now use these helpers instead — keeping one source of truth for
what "today" means for a given profile.
"""
