"""Opening and followed shows.

Merged from test_opening.py, test_shows.py; each section keeps its
original file's notes, because those carry the incident that led to the test.
"""
from unittest.mock import patch, MagicMock
from palmer import opening, page, morning, shows
from contextlib import contextmanager
from datetime import date, timedelta


# ============================================================================
# from test_opening.py
# ============================================================================
#
...
def _cache_hit_only():
    ...
    with patch("palmer.weather._geocode", side_effect=lambda c: COORDS[c]), \
         patch.object(opening, "_http_get_json", side_effect=boom), \
         patch("palmer.datafeeds._search_raw", side_effect=boom), \
...
def _offline(curated=None, tavily=None, http=None):
    ...
    ), patch("palmer.datafeeds._search_raw", return_value=tavily or []), \
        patch("palmer.weather._geocode", side_effect=lambda c: COORDS[c])
...
class TestTheCostModel:
    ...
    def test_two_users_in_one_metro_share_a_single_fetch(self):
        ...
        with patch.object(opening, "_http_get_json", return_value={}), \
             patch("palmer.weather._geocode", side_effect=lambda c: COORDS[c]), \
             patch("palmer.datafeeds._search_raw", return_value=[]) as tav, \
    ...
    def test_a_user_with_no_city_costs_nothing(self):
        opening._clear_caches()
        with patch("palmer.weather._geocode") as geo, \
             patch("palmer.datafeeds._search_raw") as tav, \
...
class TestDegradation:
    ...
    def test_a_geocode_failure_returns_nothing_rather_than_raising(self):
        opening._clear_caches()
        with patch("palmer.weather._geocode", side_effect=RuntimeError("boom")):
    ...
    def test_a_curation_failure_yields_no_rows_rather_than_raw_junk(self):
        ...
        opening._clear_caches()
        with patch("palmer.weather._geocode", side_effect=lambda c: COORDS[c]), \
             patch("palmer.datafeeds._search_raw", return_value=[{"title": "x", "url": "https://a.com/1"}]), \
...
class TestLongLeadEvents:
    ...
    def test_a_snapshot_makes_two_ticketmaster_calls(self):
        ...
        with patch.object(opening, "TICKETMASTER_API_KEY", "k"), \
             patch("palmer.weather._geocode", side_effect=lambda c: COORDS[c]), \
             patch("palmer.datafeeds._search_raw", return_value=[]), \
...
class TestExpiredRowsDropOut:
    ...
    def test_curate_stores_a_valid_date_from_the_model(self):
        ...
        opening._clear_caches()
        with patch("palmer.weather._geocode", side_effect=lambda c: COORDS[c]), \
             patch("palmer.datafeeds._search_raw",
...
class TestEpisodesLeadAndDisplace:
    ...
    def _snapshot(self, episodes):
        ...
        with patch("palmer.weather._geocode", return_value=(38.5, -90.5, "Kirkwood")), \
             patch("palmer.shows.episode_rows", return_value=episodes):
    ...
    def test_it_ignores_the_discovery_kinds_setting(self):
        ...
        opening._screen_cache[today.isoformat()] = []
        with patch("palmer.weather._geocode", return_value=(38.5, -90.5, "Kirkwood")), \
             patch("palmer.shows.episode_rows", return_value=[self.ROW]):
    ...
    def test_a_broken_shows_module_never_costs_the_section(self):
        ...
        opening._screen_cache[today.isoformat()] = []
        with patch("palmer.weather._geocode", return_value=(38.5, -90.5, "Kirkwood")), \
             patch("palmer.shows.episode_rows", side_effect=RuntimeError("boom")):
