...
class TestHeadlinesFallBackToUnvettedSources:
    ...
    def _fetch(self, trusted, untrusted):
        ...
        with patch("palmer.datafeeds._search_raw", side_effect=_search), \
             patch("palmer.morning._rotated_topics", side_effect=lambda t, d: t):
...
class TestPricesSurviveAFailure:
    ...
    def test_a_failed_symbol_keeps_its_last_row(self):
        with patch("palmer.datafeeds.price_snapshot", side_effect=[None, {"label": "NVDA", "price": 215.0}]):
    ...
    def test_a_fresh_fetch_wins_over_the_stale_row(self):
        with patch("palmer.datafeeds.price_snapshot", return_value={"label": "Bitcoin", "price": 80000.0}):
    ...
    def test_with_no_history_a_failure_simply_drops_out(self):
        ...
        with patch("palmer.datafeeds.price_snapshot", return_value=None):
    ...
    def test_a_removed_ticker_does_not_come_back_from_the_stale_set(self):
        ...
        with patch("palmer.datafeeds.price_snapshot", return_value={"label": "NVDA", "price": 215.0}):
...
class TestRebuild:
    ...
    def test_tracking_survives_a_db_failure(self):
        with patch("palmer.db.get_user_watches", side_effect=RuntimeError("db down")), \
             patch("palmer.db.get_user_price_watches", side_effect=RuntimeError("db down")), \
...
class TestOpeningDefault:
    ...
    def test_opening_fetches_by_default(self):
        with patch("palmer.opening.opening_snapshot", return_value=[{"title": "Mamele's"}]) as snap:
    ...
    def test_opening_still_fetches_for_a_profile_with_no_prefs_at_all(self):
        with patch("palmer.opening.opening_snapshot", return_value=[]) as snap:
    ...
    def test_explicit_false_opts_a_user_out(self):
        ...
        with patch("palmer.opening.opening_snapshot") as snap:
...
class TestMarketsSort:
    ...
    def _fetch(self, prefs):
        ...
        with patch("palmer.datafeeds.price_snapshot", side_effect=list(self.ROWS)):
    ...
    def test_a_stale_fallback_row_sorts_with_the_rest(self):
        ...
        with patch("palmer.datafeeds.price_snapshot",
