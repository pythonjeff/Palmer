...
class TestApply:
    def _apply(self, phone, form, city_topics=("Austin local news",
        ...
        with patch("morning.default_topics", return_value=list(city_topics)), \
             ...
             patch("agent._normalize_price_topic", side_effect=lambda t: t), \
...
class TestTopics:
    def _topics(self, city, follows, base=("National and international news",)):
        with patch("morning.default_topics", return_value=list(base)), \
             patch("agent._normalize_price_topic", side_effect=lambda t: t):
    ...
    def test_each_entry_gets_the_texted_topic_normalization(self):
        ...
        with patch("morning.default_topics", return_value=[]), \
             patch("agent._normalize_price_topic",
    ...
    def test_normalizer_failure_keeps_the_raw_topic(self):
        with patch("morning.default_topics", return_value=[]), \
             patch("agent._normalize_price_topic", side_effect=RuntimeError("yahoo down")):
...
class TestCityLandingInChat:
    ...
    def test_eager_build_replaces_a_parked_setup_form(self, store):
        ...
        with patch("home.home_token", return_value="tok"), \
             patch("home.load", return_value={"phone": "+1", "setup_pending": True}), \
