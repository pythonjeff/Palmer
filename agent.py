...
Match the register: confusion → 'John Travolta confused', celebration → 'confetti explosion', someone being dramatic → 'Sarah Jessica Parker gasp'. Never search just 'funny' or 'happy' — too generic.""",
    ...
    },
    {
        ...
        "description": "Search Google Shopping for products RIGHT NOW — a one-shot browse answer, not a persistent watch. Use when the user is discovering, browsing, gift-hunting, or asking what's available at a price point ('show me Reebok shoes around $100', 'good waterproof headphones under $150', 'find me a wool coat', 'gift ideas for a 12 year old under $50'). Extract a clean query (brand + category + any qualifiers the user mentioned). If they gave a hard cap ('under $150'), pass it as max_price. If they said 'around $100', pass min_price/max_price as a reasonable band (roughly ±30%).\n\nResults come back cheapest-first, one line per product. Each line ends with ' | <shortened URL>' — an SMS-safe TinyURL for that product.\n\nInitial recommendation reply: pick 2-3 to describe in your voice — no bullets, no dump, and do NOT include the URLs (people want the recommendation, not a link list). Also call this tool again when the user asks for a link ('send me a link to the Rockaway', 'link to the second one', 'where can I buy it?') — re-run with a narrow query targeting just that specific product, then include ONLY that one product's shortened URL in your reply. Never send more than one link in a message unless they explicitly asked for multiple. This is separate from add_price_watch (which persists) and from get_price (crypto/stock only).",
        "input_schema": {
            "type": "object",
            "properties": {
